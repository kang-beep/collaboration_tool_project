...
from chat.models import Room, PrivateRoom, PrivateMessage, TeamMessage  # chat.models Room,PrivateMessage 모델을 가져옵니다.
...
class PrivateChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # 기존 만들어진 room_id가 있는지 없는지 검증
        try:
            self.room = await sync_to_async(PrivateRoom.objects.get)(room_id=self.room_id)
        except PrivateRoom.DoesNotExist:
            self.room = await sync_to_async(PrivateRoom.objects.create)(room_id=self.room_id)
    ...
    async def receive(self, text_data):
        ...
        if sender.is_authenticated:
            ...
            if image_data:
                try:
                    ...
                    image_content = ContentFile(
                        ...
                        name=f'{sender.username}_{datetime.now().strftime("%Y%m%d%H%M%S")}.{ext}'
                    )
            ...
            try:
                ...
                if image_content:
                    message_data['image'] = image_content

                # 메시지 생성
                private_message = await sync_to_async(PrivateMessage.objects.create)(**message_data)
                
                timestamp = await sync_to_async(lambda: private_message.timestamp.strftime("%Y-%m-%d %H:%M:%S"))()
                image_url = private_message.image.url if private_message.image else None
